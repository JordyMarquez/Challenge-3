...
var passwordCharCount = 0  
var shouldUseLowerCase = true
...
// Sets parameter to be between 8 & 128 characters
   passwordCharCount = prompt("How many characters would you like to use? Must be between 8 and 128 characters")
...
// Asks user to select password criteria and creates an array
   shouldUseLowerCase = confirm("Do you want to use lower case letters?")
   if (shouldUseLowerCase) {
    userChoiceArray = userChoiceArray.concat(lowercaseArray)
    console.log(userChoiceArray)
   }
   
   shouldUseUpperCase = confirm("Do you want to use upper case letters?")
   if (shouldUseUpperCase) {
    userChoiceArray = userChoiceArray.concat(uppercaseArray)
   }
   console.log(userChoiceArray)

   shouldUseSymbols = confirm("Do you want to use symbols?")
   if (shouldUseSymbols) {
    userChoiceArray = userChoiceArray.concat(symbols)
   }
   console.log(userChoiceArray)

   shouldUseNumbers = confirm("Would you like to use numbers?")
   if (shouldUseNumbers) {
